...
const editMap = (options) => {
  const queryParams = [];
  const setClause = [];
  const {map_id, name, description, header_image, public} = options;
  const queryString = `UPDATE table_name `;
...
// Creat a new map >> Add
const addMap = (map) => {
  const {user_id, name, description, header_image, date_created, date_modified, public} = map;
  const queryString = `
  INSERT INTO maps (user_id, name, description, header_image, date_created, date_modified, public)
  VALUE ($1, $2, $3, $4, $5, $6, $7)
  RETURNING *;
  `
  const queryParams = [user_id, name, description, header_image, date_created, date_modified, public];
