...
const getSelectedPin = (pin_id) => {
  const queryString = `
  SELECT * FROM pins
  WHERE pins.id = $1
  `
  console.log(pin_id);
  return db.query(queryString, [pin_id])
  .then(result => result.rows)
  .catch(err => console.log(err.message));
}
...
const editPin = (pinDetails) => {
  const queryParams = [];
  const setClause = [];
  const {pin_id, pin_icon_id, name, description, location, locked} = pinDetails;
  ...
  queryString += ` WHERE pins.id = ${pin_id}
                   RETURNING *;`;
...
const addPin = (pinDetails) => {
  const {user_id, name, description, header_image, public} = pinDetails;
  const queryString = `
  INSERT INTO pins (user_id, name, description, header_image, date_created, date_modified, public)
...
const deletePin = (pin_id, user_id) => {
  const queryString = `
  DELETE FROM pins
  WHERE pins.id = $1
  AND user_id = $2
