...
const getAllPinsFromAllMaps = () => {
  ...
  .then((result)=>{
    ...
    const markerOptions = {
      alt: result.rows.name,
      title: result.rows.name,
      keyboard: true,
      draggable: true,
      riseOnHover: true,
      closeButton: true
    ...
    const popupFormat = `
        <h1>${result.rows.name}</h1>
        <p>${result.rows.description}</p>
        <img src="/styles/condensed_image/uploads/${result.rows.image_url}">
        <button id="editPin">Edit Pin</button> <button id="deletePin">Delete Pin</button>
    `;
    ...
    // console.log('long', geolocation[1])
    document.write(
      unescape("%3Cscript src='https://code.jquery.com/jquery-3.6.0.min.js' type='text/javascript'%3E%3C/script%3E")
    );
